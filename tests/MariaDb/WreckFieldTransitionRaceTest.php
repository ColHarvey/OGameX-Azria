 ...
 * enfants, et ne le relache que lorsque la base montre les deux arretes dessus. **Avant la correction**, une commande
 * n'est arretee qu'a sa premiere ecriture : ses controles ont deja passe sur l'etat d'avant, et la relache fait
 * ecrire les deux. **Apres**, une commande commence par relire sous verrou : elle est arretee avant de decider, et la
 * relache fait decider l'une puis l'autre, sur l'etat que la premiere a laisse. Une meme attente — « arrete au moins
 * une seconde sur une instruction qui touche la table » — couvre les deux mondes.
 *
 * ## L'ordre des verrous
 *
 * Le reglement d'une bataille tient la planete cible avant d'ecrire l'epave (planete, puis champ). Une recuperation qui
 * prenait le champ puis la planete formait un cycle avec lui : MariaDB tranche un interblocage en tuant l'un des deux.
 * Le dernier temoin le joue et exige qu'aucun des deux ne meure.
 */
#[Group('mariadb')]
final class WreckFieldTransitionRaceTest extends AccountTestCase
{
    ...
    public function testTwoSimultaneousStartsOnTheSameWreckStartOneRepairAndWriteOneTimer(): void
    {
        ...
        $issues = $this->inParallel(
            ...
            function () use ($temoin): void {
                $this->waitUntilProcessesAreStoppedOn('wreck_fields', 2);
                $temoin->commit();
            }
        );
    ...
    public function testTwoStartsOnTwoWrecksOfTheSameOwnerAtOnceLeaveOneRepairing(): void
    {
        ...
        $issues = $this->inParallel(
            ...
            function () use ($temoin, $seconde): void {
                // Avant la correction, le demarrage de la seconde epave n'attend personne : il est deja ecrit
                // quand le premier est arrete. Apres, les deux relisent l'ensemble du proprietaire et attendent.
                $this->waitUntilProcessesAreStoppedOn('wreck_fields', 1);
                $this->waitUntilProcessesAreStoppedOn('wreck_fields', 2, fn (): bool => $this->statusOf($seconde) === 'repairing');
                $temoin->commit();
            }
        );
     ...
     *
     * Le parent tient la planete : la recuperation, qui la verrouille, s'arrete ; le brulage, lui, n'a besoin que de
     * l'epave. Avant la correction, la recuperation tenait deja l'epave et le brulage s'arretait a son ecriture ; relache,
     * la recuperation deployait et effacait l'epave, puis le brulage ecrivait sur une ligne disparue — et se disait fait.
     */
    public function testACollectionAndABurnAtOnceDeployOrBurnTheShipsNeverBoth(): void
    {
        ...
        $issues = $this->inParallel(
            ...
            function () use ($temoin, $champ): void {
                $this->waitUntilProcessesAreStoppedOn('planets', 1);
                $this->waitUntilProcessesAreStoppedOn('wreck_fields', 1, fn (): bool => $this->statusOf($champ) === 'burned');
                $temoin->commit();
            }
        );
    ...
    /**
     * Le reglement d'une bataille tient la planete puis ecrit l'epave ; une recuperation qui prend l'epave puis la
     * planete forme un cycle avec lui. Aucun des deux ne doit mourir.
     */
     ...
     * touche cette table — une lecture verrouillante comme une ecriture —, ou que `$ouBien` soit vrai.
     *
     * @param Closure(): bool|null $ouBien
     */
    private function waitUntilProcessesAreStoppedOn(string $table, int $combien, Closure|null $ouBien = null, int $timeoutMs = 15_000): void
    {
        ...
        $this->fail("Fewer than {$combien} processes came to a stop on {$table}: the race would not be forced. Seen:\n" . implode("\n", $vu));
    }
